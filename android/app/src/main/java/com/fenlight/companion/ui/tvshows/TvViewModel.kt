...
class TvViewModel(application: Application) : AndroidViewModel(application) {
                ...
                configs.map { config ->
                    async {
                        ...
                        try {
                            ...
                            val items = result.results.map { show ->
                                PaginatedItem(
    ...
    private suspend fun fetchPage1ForConfig(config: BrowseRowConfig, region: String?, excludeAdult: Boolean) =
        when (config.type) {
            ...
            // Movie-only types — shouldn't appear in TV rows, fall back to popular
            RowType.NOW_PLAYING, RowType.UPCOMING -> app.tmdbApi.popularTv(1, region)
        }
